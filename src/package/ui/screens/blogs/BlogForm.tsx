...
import type { BlogFormValues } from '@/features/blogs/model/blog.schema';
    ...
    <div className="space-y-4 max-w-none">
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
          ...
          >
            {isSaving ? <LoaderIcon /> : <Send className="w-4 h-4" />}
            {isSaving ? '保存中...' : status === 'published' ? '更新发布' : '立即发布'}
          </button>
        </div>
      </div>
      ...
      <div className="flex gap-5 items-start">
        <div className="flex-1 min-w-0 space-y-4">
          ...
          <div className="bg-white rounded-xl border border-gray-200 p-5 space-y-4">
            <div>
              <label className={FIELD_LABEL}>
                中文摘要 <span className="text-gray-300 font-normal">(限 160 字)</span>
              </label>
              <Controller
                name="summary.zh"
                control={form.control}
              ...
              />
              <p className="text-right text-xs text-gray-400">{(form.watch('summary.zh') || '').length}/160</p>
            </div>
            <div className="border-t border-gray-100 pt-4">
              <label className={FIELD_LABEL}>
                English Summary <span className="text-gray-300 font-normal">(max 160 chars)</span>
              </label>
              <Controller
                name="summary.en"
                control={form.control}
              ...
              />
              <p className="text-right text-xs text-gray-400">{(form.watch('summary.en') || '').length}/160</p>
            </div>
          </div>
          ...
          <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
            ...
            <div className="p-4">
              <Controller
                name={activeTab === 'zh' ? 'content.zh' : 'content.en'}
                control={form.control}
                render={({ field }) => (
                  <MarkdownEditor
                    value={field.value || ''}
                    ...
                    placeholder={
                      activeTab === 'zh'
                        ? '在此输入中文正文（支持 Markdown 格式，可粘贴图片自动上传）...'
                        : 'Enter English body content here (Markdown supported, paste images to auto-upload)...'
                    }
                  />
                )}
              />
        ...
        <div className="w-72 flex-shrink-0 space-y-3 sticky top-4">
          <div className={SECTION_CLASS}>
            ...
            <div>
              <label className={FIELD_LABEL}>
                <Calendar className="w-3 h-3 inline mr-1" />发布日期
              </label>
              <Controller
                name="publishDate"
                control={form.control}
            ...
            <div>
              ...
              <Controller
                ...
                control={form.control}
                render={({ field }) => (
                  <input {...field} type="text" className={INPUT_CLASS} placeholder="Admin" />
                )}
              />
            </div>
          </div>
              ...
              <div className="space-y-3 pt-2">
                <div>
                  ...
                  <Controller
                    ...
                    control={form.control}
                    render={({ field }) => (
                      <input {...field} type="text" className={INPUT_CLASS} placeholder="默认使用文章标题" />
                    )}
                  />
                </div>
                <div>
                  ...
                  <Controller
                    ...
                    control={form.control}
                    render={({ field }) => (
                      <input {...field} type="text" className={INPUT_CLASS} placeholder="Defaults to article title" />
                    )}
                  />
                </div>
                <div>
