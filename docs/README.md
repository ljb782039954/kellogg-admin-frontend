...
├── reviews/                     客户评价列表与表单
├── inquiries/                   询盘收件箱（服务端搜索/分页）+ system-inquiry 页面配置
└── build/                       前台构建触发与状态管理
shared/
...
当前 `/company`、`/categories`、`/header`、`/footer`、`/products`、`/pages`、`/media`、`/reviews`、`/inquiries`、`/inquiry-editor` 路由已迁移到对应 `features/`；构建触发也已迁入 `features/build`。
